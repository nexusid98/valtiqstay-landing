...
export default async function DashboardPage() {
  ...
  const conversionRate = active.length > 0 ? Math.round((checkedIn / active.length) * 100) : 0;
  ...
  const avgNights =
    active.length > 0
      ? (active.reduce((s, r) => s + nightsBetween(r.arrival, r.departure), 0) / active.length).toFixed(1)
      : "—";

  const now = new Date();
  const in7 = new Date(now.getTime() + 7 * 86_400_000);
  const upcomingArrivals = active.filter((r) => {
    const a = new Date(r.arrival + "T12:00:00");
    return a >= now && a <= in7 && r.status === "pending";
  }).length;
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
        ...
        {/* Table */}
        <ReservationsTable reservations={all} />
        {/* OTA Integration */}
        {hotelData?.api_key && (
          <div
            style={{
